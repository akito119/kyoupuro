...
lint main() {
    int n,m;
    cin>>n>>m;
    Segtree lar(n,op,(pair<int,int>){0,0});
    Segtree mini(n, op2, (pair<int,int>) {LLONG_MAX, 0});
    rep(i, n){
        int p;
        cin>>p;
        lar.insert(i, {p, i});
    ...
    rep(i, m){
        ...
        cin>>l>>r;
        l --;
        pair<int,int> largeNum = lar.query(l, r);
        pair<int,int> miniNum = mini.query(l,r);
        
        lar.insert(largeNum.second, {miniNum.first,largeNum.second});
        lar.insert(miniNum.second, {largeNum.first, miniNum.second});
        mini.insert(largeNum.second, {miniNum.first,largeNum.second});
        mini.insert(miniNum.second, {largeNum.first, miniNum.second});
    }

    rep(i, n){
        cout<<lar.value(i).first<<endl;
    }
}